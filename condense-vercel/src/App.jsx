...
// ─── SUPABASE CONFIG ──────────────────────────────────────────────────────────
const SUPA_URL = "https://btcmmpobupyofgptnswl.supabase.co";
const SUPA_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ0Y21tcG9idXB5b2ZncHRuc3dsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMzMDIzNzgsImV4cCI6MjA4ODg3ODM3OH0.FbI7BiWpIG8k2RlWZQEG62LV6yPSYkZO-f6FgH9hXrI";

const supa = {
  headers: { "apikey": SUPA_KEY, "Authorization": `Bearer ${SUPA_KEY}`, "Content-Type": "application/json" },

  async signUp(email, password) {
    const r = await fetch(`${SUPA_URL}/auth/v1/signup`, { method: "POST", headers: this.headers, body: JSON.stringify({ email, password }) });
    return r.json();
  },
  async signIn(email, password) {
    const r = await fetch(`${SUPA_URL}/auth/v1/token?grant_type=password`, { method: "POST", headers: this.headers, body: JSON.stringify({ email, password }) });
    return r.json();
  },
  async signOut(token) {
  ...
  async getUser(token) {
    const r = await fetch(`${SUPA_URL}/auth/v1/user`, { headers: { ...this.headers, "Authorization": `Bearer ${token}` } });
    return r.json();
  },
  ...
  async getProspects(token) {
    const r = await fetch(`${SUPA_URL}/rest/v1/prospects?order=created_at.asc`, { headers: this.authHeaders(token) });
    return r.json();
  },
  async upsertProspect(token, p) {
    await fetch(`${SUPA_URL}/rest/v1/prospects`, {
      method: "POST", headers: { ...this.authHeaders(token), "Prefer": "resolution=merge-duplicates,return=minimal" },
      body: JSON.stringify({ id: p.id, user_id: p.user_id, name: p.name, job_title: p.jobTitle, company: p.company, email: p.email || null, phone: p.phone || null, linkedin_url: p.linkedinUrl || null, industry: p.industry || null, seniority: p.seniority || null, region: p.region || null, status: p.status, created_at: p.createdAt, sent_at: p.sentAt || null }),
    });
  },
  async deleteProspect(token, id) {
    await fetch(`${SUPA_URL}/rest/v1/prospects?id=eq.${id}`, { method: "DELETE", headers: this.authHeaders(token) });
  },

  async getResearch(token) {
    const r = await fetch(`${SUPA_URL}/rest/v1/research?select=prospect_id,data`, { headers: this.authHeaders(token) });
    const rows = await r.json();
    return Object.fromEntries((rows || []).map(r => [r.prospect_id, r.data]));
  },
  async upsertResearch(token, prospectId, userId, data) {
    await fetch(`${SUPA_URL}/rest/v1/research`, {
      method: "POST", headers: { ...this.authHeaders(token), "Prefer": "resolution=merge-duplicates,return=minimal" },
      body: JSON.stringify({ prospect_id: prospectId, user_id: userId, data }),
    });
  },

  async getMessages(token) {
    const r = await fetch(`${SUPA_URL}/rest/v1/messages?select=prospect_id,data,edits`, { headers: this.authHeaders(token) });
    const rows = await r.json();
    const msgs = {}, edits = {};
    (rows || []).forEach(r => { msgs[r.prospect_id] = r.data; edits[r.prospect_id] = r.edits || {}; });
    return { msgs, edits };
  },
  async upsertMessages(token, prospectId, userId, data, edits) {
    await fetch(`${SUPA_URL}/rest/v1/messages`, {
      method: "POST", headers: { ...this.authHeaders(token), "Prefer": "resolution=merge-duplicates,return=minimal" },
      body: JSON.stringify({ prospect_id: prospectId, user_id: userId, data, edits: edits || {} }),
    });
  },
};
...
export default function App() {
  ...
  const [session, setSession] = useState(() => {
    try { return JSON.parse(localStorage.getItem("co_session") || "null"); } catch { return null; }
  });
  ...
  const handleAuth = async () => {
    ...
    try {
      const res = authMode === "login"
        ? await supa.signIn(authEmail, authPassword)
        : await supa.signUp(authEmail, authPassword);
      if (res.error) { setAuthError(res.error.message || "Auth failed"); return; }
      if (res.access_token) {
        const s = { access_token: res.access_token, user: res.user };
        setSession(s);
        localStorage.setItem("co_session", JSON.stringify(s));
  ...
  const handleSignOut = async () => {
    ...
    setProspects([]); setResearch({}); setMessages({}); setEdits({});
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [pros, res, { msgs, edits: eds }] = await Promise.all([
          supa.getProspects(token),
          supa.getResearch(token),
          supa.getMessages(token),
        ]);
        if (Array.isArray(pros)) {
          setProspects(pros.map(p => ({
            id: p.id, name: p.name, jobTitle: p.job_title, company: p.company,
            email: p.email, phone: p.phone, linkedinUrl: p.linkedin_url,
            industry: p.industry, seniority: p.seniority, region: p.region,
            status: p.status, createdAt: p.created_at, sentAt: p.sent_at, user_id: p.user_id,
          })));
        }
        setResearch(res || {});
        setMessages(msgs || {});
    ...
    return (
      <>
        ...
        <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: FONT }}>
          <div style={{ width: 380, padding: 36, background: C.surface, border: `1px solid ${C.borderBright}`, borderRadius: 8, boxShadow: `0 0 60px rgba(14,165,233,0.1)` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
              <div style={{ width: 36, height: 36, borderRadius: 6, background: "linear-gradient(135deg, #0a1535, #050d20)", border: `1px solid ${C.gold}55`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: 15, fontFamily: DISPLAY, color: C.gold, fontWeight: 700 }}>C</span>
              </div>
              <div>
                <div style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 16, color: C.text }}>Condense Outreach</div>
                <div style={{ fontSize: 10, color: C.textDim, fontFamily: MONO, letterSpacing: "0.08em" }}>AGENTIC · RESEARCH · PERSONALIZED</div>
              </div>
            </div>

            <div style={{ display: "flex", marginBottom: 24, borderBottom: `1px solid ${C.borderDim}` }}>
              {["login", "signup"].map(m => (
                <button key={m} onClick={() => { setAuthMode(m); setAuthError(""); }} style={{
                  flex: 1, padding: "10px", border: "none", background: "none",
                  borderBottom: authMode === m ? `2px solid ${C.gold}` : "2px solid transparent",
                  color: authMode === m ? C.text : C.textDim,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <label style={{ fontSize: 10, color: C.textDim, fontFamily: MONO, letterSpacing: "0.1em", display: "block", marginBottom: 6 }}>EMAIL</label>
                <input type="email" value={authEmail} onChange={e => setAuthEmail(e.target.value)}
                  placeholder="you@example.com" onKeyDown={e => e.key === "Enter" && handleAuth()}
                  style={{ width: "100%", background: "rgba(0,0,0,0.5)", border: `1px solid ${C.border}`, color: C.text, borderRadius: 4, padding: "10px 12px", fontSize: 13, fontFamily: FONT, outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ fontSize: 10, color: C.textDim, fontFamily: MONO, letterSpacing: "0.1em", display: "block", marginBottom: 6 }}>PASSWORD</label>
              ...
              {authError && (
                <div style={{ fontSize: 11, fontFamily: MONO, padding: "8px 12px", borderRadius: 4,
                  background: authError.startsWith("✅") ? C.greenDim : C.redDim,
                  color: authError.startsWith("✅") ? C.green : C.red,
                }}>{authError}</div>
              )}

              <button onClick={handleAuth} disabled={authLoading || !authEmail || !authPassword} style={{
                padding: "11px", borderRadius: 4, border: `1px solid ${C.gold}`,
                background: `linear-gradient(135deg, rgba(14,165,233,0.2), rgba(14,165,233,0.08))`,
                color: C.goldBright, fontWeight: 600, fontSize: 13, fontFamily: FONT,
                cursor: authLoading ? "not-allowed" : "pointer", opacity: (!authEmail || !authPassword) ? 0.4 : 1,
              }}>
  ...
  // Auto-sync prospects to Supabase whenever they change
  const syncProspect = useCallback(async (p) => {
    if (!token || !userId) return;
    await supa.upsertProspect(token, { ...p, user_id: userId });
  }, [token, userId]);

  const syncMessages = useCallback(async (prospectId, data, currentEdits) => {
    if (!token || !userId) return;
    await supa.upsertMessages(token, prospectId, userId, data, currentEdits);
  }, [token, userId]);

  const syncResearch = useCallback(async (prospectId, data) => {
    if (!token || !userId) return;
    await supa.upsertResearch(token, prospectId, userId, data);
  }, [token, userId]);
  ...
  const addProspect = () => {
    ...
    const newP = { ...form, id, user_id: userId, status: "idle", createdAt: new Date().toISOString(), sentAt: null };
  ...
  const runAgent = async (prospect) => {
    ...
    try {
      ...
      const cleaned = cleanResearch(researchData);
      setResearch(prev => ({ ...prev, [id]: cleaned }));
      syncResearch(id, cleaned);
  ...
  const markSent = (id) => {
    setProspects(prev => prev.map(p => {
      if (p.id !== id) return p;
      const updated = { ...p, status: "following", sentAt: new Date().toISOString() };
      syncProspect(updated);
      return updated;
    }));
  };
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 4 }}>
              <span style={{ fontSize: 10, fontFamily: MONO, color: C.textDim }}>{session?.user?.email?.split("@")[0]}</span>
              <button onClick={handleSignOut} style={{ fontSize: 10, fontFamily: MONO, color: C.red, background: "none", border: `1px solid ${C.red}44`, borderRadius: 3, padding: "4px 8px", cursor: "pointer" }}>Sign Out</button>
            </div>
          </div>
        </div>
                        ...
                        <div style={{ background: "rgba(0,0,0,0.3)", border: `1px solid ${C.borderDim}`, borderTop: "none", borderRadius: "0 0 4px 4px", padding: 20 }}>
                          ...
                          <textarea
                            ...
                            onChange={e => {
                              ...
                              setEdits(prev => {
                                const next = { ...prev, [editKey]: val };
                                syncMessages(sel.id, selMessages, next);
                                return next;
                              });
                            }}
                            style={{
